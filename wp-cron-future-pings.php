...
// stuff all the sites to ping into the `pinged` field, and blank `to_ping`
$result = $wpdb->query("UPDATE $wpdb->posts SET pinged = '$to_ping', to_ping = '' WHERE ID=$the_ID");
...
foreach ($future_pings as $ID => $date)
{
	if ($date < current_time('mysql'))
	{
		...
		$result = $wpdb->query("UPDATE $wpdb->posts SET to_ping = '$pings', pinged='' WHERE ID=$ID");
		generic_ping($ID);
		if (0 !== get_option('default_pingback_flag'))
		{
