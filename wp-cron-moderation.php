...
$queue = $wpdb->get_results("SELECT comment_ID, comment_post_ID FROM $wpdb->comments WHERE comment_approved = '0' ORDER BY comment_post_ID ASC");

$comment_count = count($queue);

if (0 == $comment_count) return;

$comments_posts = array_unique(array_values($queue[0]));
$comment_count_posts = count($comments_posts);
